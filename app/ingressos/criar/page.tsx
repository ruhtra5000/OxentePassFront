...
export default function NovoIngressoPage() {
  ...
  useEffect(() => {
    async function carregarEventos() {
      setCarregando(true);
      try {
        const data = await chamadaAPI("/evento", "GET");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      tipoIngresso: formData.tipoIngresso,
              ...
              <div className="p-8 space-y-6">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs font-bold bg-white/20 px-3 py-1 rounded-full inline-block mt-1 uppercase tracking-tight">Novo Lote</p>
                  </div>
                </div>
          ...
          <div className="lg:col-span-7 bg-white rounded-[3rem] border border-slate-100 shadow-sm p-8 md:p-12 relative overflow-hidden">
            {carregando ? (
              <div className="py-20 flex flex-col items-center justify-center space-y-4">
                ...
                <span className="font-black text-slate-300 uppercase tracking-widest text-xs">Sincronizando...</span>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-10">
                ...
                <section className="space-y-4">
                  <div className="flex items-center gap-3 ml-1">
                    ...
                    <label className="font-black text-slate-500 uppercase tracking-[0.2em] text-[12px]">Evento de Destino</label>
                  </div>
                  <select
                    required
                    value={formData.eventoId}
                    onChange={(e) => setFormData({...formData, eventoId: e.target.value})}
                    className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-5 outline-none focus:border-emerald-500 focus:bg-white transition-all font-bold text-slate-700 appearance-none"
                  >
                ...
                <div className="pt-4">
                  ...
                  >
                    {salvando ? "Sincronizando..." : "Confirmar e Publicar Lote"}
                  </button>
                </div>
              </form>
            )}
